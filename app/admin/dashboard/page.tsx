  ...
  const positionResults = positions
    ...
    .map(position => {
      ...
      const candidatesWithRanks = candidatesWithVotes.map((candidate, index) => {
        ...
        if (index < candidatesWithVotes.length - 1 && candidate.votes === candidatesWithVotes[index + 1].votes) {
          isTied = true
        }
...
export default async function AdminDashboard() {
  ...
  const { election, stats, positionResults, turnoutByLevel } = await getDashboardData(associationId)
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <Card className="w-full max-w-md text-center">
          <CardContent className="pt-6">
            ...
            <p className="text-muted-foreground mb-4">There are no active elections at this time.</p>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-foreground">{stats.totalVoters.toLocaleString()}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-foreground">{stats.totalVotes.toLocaleString()}</div>
              <p className="text-xs text-muted-foreground mt-1">
                {stats.totalVotes > 0 ? '+' : ''}{stats.totalVotes} votes recorded
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">Across {stats.totalPositions} positions</p>
            </CardContent>
          </Card>
          
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Turnout Rate</CardTitle>
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                {stats.totalVotes.toLocaleString()} of {stats.totalVoters.toLocaleString()} voters have participated
              </p>
            </CardHeader>
            <CardContent>
              <TurnoutChart 
